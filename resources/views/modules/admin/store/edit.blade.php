...
@section('content')
    <x-breadcrumb>
        ...
        <x-breadcrumb.item href="#">Edit</x-breadcrumb.item>
    </x-breadcrumb>
    <x-module x-data="store()" x-init="setStoreProductData({{ json_encode($product) }})">
        <x-module.header>
                ...
                <x-form x-on:submit.prevent="handleUpdate()">
                    ...
                    <x-form.group>
                        ...
                        <x-select x-model="form.data.type" id="type">
                            <option value="credits" x-bind:selected="form.data.type == 'credits'">
                                Credits
                            </option>
                            <option value="slots" x-bind:selected="form.data.type == 'slots'">
                                Server Slots
                            </option>
                        </x-select>
                    ...
                    <x-form.group>
                        ...
                        <x-select x-model="form.data.category_id" id="categoryId">
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" x-bind:selected="form.data.category_id == {{ $category->id }}">
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    <x-form.group>
                        ...
                        <x-select x-model="form.data.active" id="active">
                            <option value="1" x-bind:selected="form.data.active == '1'">
                                Yes
                            </option>
                            <option value="0" x-bind:selected="form.data.active == '0'">
                                No
                            </option>
                        </x-select>
